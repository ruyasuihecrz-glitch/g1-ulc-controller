...
def test_curriculum_range():
    ...
    assert closed["height"] == (0.72, 0.75)
    assert open_["height"] == (0.30, 0.75)
    assert closed["torso_yaw"] == (-0.0, 0.0)
    assert open_["torso_yaw"] == (-2.62, 2.62)


def test_curriculum_range_supports_asymmetric_torso_bounds():
    ranges = ulc_curriculum_ranges(
        default_height=0.74,
        alpha_height=0.5,
        alpha_upper=0.5,
        torso_yaw_range=(-1.0, 2.0),
        torso_roll_range=(-0.2, 0.6),
    )

    assert ranges["height"][0] >= 0.30
    assert ranges["height"][1] <= 0.75
    assert ranges["torso_yaw"] == (-0.5, 1.0)
    assert ranges["torso_roll"] == (-0.1, 0.3)
