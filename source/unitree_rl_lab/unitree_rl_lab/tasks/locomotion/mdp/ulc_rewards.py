...
from isaaclab.utils.math import matrix_from_quat, quat_inv, quat_mul, wrap_to_pi
...
def _torso_zxy(env, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    torso_quat_root = quat_mul(quat_inv(asset.data.root_quat_w), asset.data.body_quat_w[:, body_id])
    return _zxy_from_quat(torso_quat_root)
...
def ulc_track_com_exp(
    ...
    asset = _robot(env, asset_cfg)
    masses = asset.root_physx_view.get_masses().to(asset.data.body_pos_w.device)
    if masses.dim() == 1 and masses.shape[0] == asset.num_bodies:
        masses = masses.unsqueeze(0).expand(env.num_envs, -1)
    if masses.shape != asset.data.body_pos_w.shape[:2]:
        raise RuntimeError(
            "ULC CoM reward expected masses shape to match body positions: "
            f"masses={tuple(masses.shape)}, body_pos_w={tuple(asset.data.body_pos_w.shape)}, "
            f"body_names={asset.body_names}"
        )
