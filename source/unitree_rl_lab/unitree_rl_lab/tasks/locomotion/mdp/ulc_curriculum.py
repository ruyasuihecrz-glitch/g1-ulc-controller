...
def ulc_sequential_curriculum(
    ...
    sampling ranges. Per-env resets do not reset these alphas.
    """
    ...
    last_step = getattr(command_term, "_last_curriculum_step", -update_interval_steps)
    if env.common_step_counter - last_step < update_interval_steps:
        return torch.tensor(command_term.alpha_upper, device=env.device)
    ...
    c3 = bool(r_upper >= 0.8 and r_torso >= 0.8 and c2 and command_term.alpha_height >= 0.98)
    if c3:
