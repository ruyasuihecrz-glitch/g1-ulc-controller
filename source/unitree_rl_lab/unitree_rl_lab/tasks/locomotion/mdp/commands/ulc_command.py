...
class UniformULCCommand(CommandTerm):
    ...
    def __init__(self, cfg: "UniformULCCommandCfg", env):
        ...
        self._default_root_height = min(
            max(self._default_root_height, float(self.cfg.ranges.root_height[0])),
            float(self.cfg.ranges.root_height[1]),
        )
    ...
    def _curriculum_ranges(self) -> tuple[tuple[float, float], tuple[tuple[float, float], ...]]:
        ...
        height_min = min(max(float(height_min), float(ranges.root_height[0])), float(ranges.root_height[1]))
        height_max = min(max(float(height_max), float(ranges.root_height[0])), float(ranges.root_height[1]))
        torso_yaw = (
            self._lerp(0.0, ranges.torso_yaw[0], alpha_upper),
            self._lerp(0.0, ranges.torso_yaw[1], alpha_upper),
        )
        torso_roll = (
            self._lerp(0.0, ranges.torso_roll[0], alpha_upper),
            self._lerp(0.0, ranges.torso_roll[1], alpha_upper),
        )
        torso_pitch = (
            self._lerp(0.0, ranges.torso_pitch[0], alpha_upper),
            self._lerp(0.0, ranges.torso_pitch[1], alpha_upper),
        )
        return (height_min, height_max), (torso_yaw, torso_roll, torso_pitch)
