...
RUN_NAME="${RUN_NAME:-g1_29dof_stable_velocity_formal}"
RESUME="${RESUME:-false}"
LOAD_RUN="${LOAD_RUN:-}"
CHECKPOINT="${CHECKPOINT:-}"
SESSION_NAME="g1_stable_velocity_train"
TIMESTAMP="$(date +%Y-%m-%d_%H-%M-%S)"
LOG_DIR="${REPO_ROOT}/logs"
LOG_FILE="${LOG_DIR}/g1_29dof_stable_velocity_formal_${TIMESTAMP}.log"
...
echo "resume: ${RESUME}"
echo "load_run: ${LOAD_RUN:-<latest-matching>}"
echo "checkpoint: ${CHECKPOINT:-<latest-matching>}"
...
tmux new-session -d -s "${SESSION_NAME}" "cd ${REPO_ROOT} && export PYTHONPATH='${PYTHONPATH}' PATH='${PATH}' && ${CMD} 2>&1 | tee ${LOG_FILE}"
