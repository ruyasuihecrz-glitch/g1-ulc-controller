...
RUN_NAME="${RUN_NAME:-g1_29dof_turn_finetune_from_14000}"
RESUME="${RESUME:-true}"
LOAD_RUN="${LOAD_RUN:-2026-05-28_03-07-09_g1_29dof_official_velocity_env8192_ckpt2000}"
CHECKPOINT="${CHECKPOINT:-model_14000.pt}"
SESSION_NAME="${SESSION_NAME:-g1_turn_finetune_train}"
TIMESTAMP="$(date +%Y-%m-%d_%H-%M-%S)"
LOG_DIR="${REPO_ROOT}/logs"
LOG_FILE="${LOG_DIR}/g1_29dof_turn_finetune_${TIMESTAMP}.log"
...
if is_true "${RESUME}"; then
  if [[ -z "${LOAD_RUN}" || -z "${CHECKPOINT}" ]]; then
    echo "RESUME=true requires LOAD_RUN and CHECKPOINT."
    exit 1
  fi
...
echo "turn_ft: official G1 velocity recipe + heavier yaw-rate sampling/reward"
